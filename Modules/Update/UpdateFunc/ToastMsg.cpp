...
//DebugLogOutput g_DbgLogOutput2(L"UpdateFunc.log");
...
HRESULT SetImageSrc(_In_z_ wchar_t *imagePath, _In_ IXmlDocument *toastXml)
{
	...
	if (SUCCEEDED(hr))
	{
		...
		hr = toastXml->GetElementsByTagName(StringRefWarper(L"image").Get(), &nodeList);
		if (SUCCEEDED(hr))
		{
			...
			if (SUCCEEDED(hr))
			{
				...
				if (SUCCEEDED(hr))
				{
					...
					hr = attributes->GetNamedItem(StringRefWarper(L"src").Get(), &srcAttribute);
					if (SUCCEEDED(hr))
					{
						hr = SetNodeValueString(StringRefWarper(imageSrc).Get(), srcAttribute.Get(), toastXml);
					}
				}
			}
		}
...
void CToastMsg::DisplayToast(int xmlIdx)
{
	CoInitialize(NULL);

	ComPtr<IToastNotificationManagerStatics> toastStatics;
	//Step 1 Create toast manager.
	HRESULT hr = GetActivationFactory(StringRefWarper(RuntimeClass_Windows_UI_Notifications_ToastNotificationManager).Get(), &toastStatics);
	if (SUCCEEDED(hr))
	{
		//Step 2 Create Notifier
		ComPtr<IToastNotifier> notifier;
		hr = toastStatics->CreateToastNotifierWithId(StringRefWarper(L"OMEN Command Center").Get(), &notifier);
		if (SUCCEEDED(hr))
		{
			...
			hr = GetActivationFactory(StringRefWarper(RuntimeClass_Windows_UI_Notifications_ToastNotification).Get(), &factory);
			if (SUCCEEDED(hr))
			{
	...
	::StringCchPrintfW(g_DbgLogOutput.strDebugLogOutputString, DEBUG_MAX_STRING, L"%s::Hresult %d", __FUNCTIONW__, hr);
	g_DbgLogOutput.Output();
}


HRESULT CToastMsg::GetButtonToastXml(ABI::Windows::Data::Xml::Dom::IXmlDocument ** inputXml)
{
	...
	HRESULT hr = Windows::Foundation::ActivateInstance(StringRefWarper(RuntimeClass_Windows_Data_Xml_Dom_XmlDocument).Get(), &xmlDocument);
	if (SUCCEEDED(hr))
	{
...
HRESULT CToastMsg::GetDefaultToastXml(ABI::Windows::Data::Xml::Dom::IXmlDocument ** inputXml)
{
	HStringReference toastXML(
		L"<toast scenario=\"reminder\">"
		L" <visual version=\"1\">"
...
HRESULT CToastMsg::TestGetXml(int xmlIdx, LPCWSTR xmlStr, ABI::Windows::Data::Xml::Dom::IXmlDocument ** inputXml)
{
	//size_t lenSz = wcslen(xmlStr);
	HStringReference toastXML(
		L"<toast scenario=\"reminder\">"
		L" <visual version=\"1\">"
		//L" <binding template=\"ToastGeneric\">"
